...
<html><head><title>品牌信息管理<%=Action %></title>
...
<table width='100%' border='0' align='center' cellpadding='2' cellspacing='1' class='border'>
  <form name='searchmyform' action='Admin_Brand.jsp' method='get'>
  <tr class='tdbg'>
    <td height='30'>
        &nbsp;&nbsp;&nbsp;&nbsp;<a href='Admin_Brand.jsp'>品牌信息管理首页</a>&nbsp;|&nbsp;<a href='Admin_Brand.jsp?Action=Add'>添加品牌信息</a><%=Action %>
    </td>
  </tr>
  </form>
</table>
...
recordCount = count.getRecordCount("tbbrand",type);

PageBean pageView = new PageBean(recordCount,Integer.parseInt(pages));



BrandAction getBrandList =new BrandAction();
List BrandList = new ArrayList();


BrandList = getBrandList.getBrandListAction(Keyword,pageView.getFromIndex(),pageView.getPageSize());
 %>
    <table width='100%' border='0' cellpadding='0' cellspacing='0'>
      <tr>
      <form name='myform' method='Post' action='Admin_Brand.jsp'>
          <td>
          <table width='100%' border='0' align='center' cellpadding='2' cellspacing='1' class='border'>
        ...
        <tr><td colspan='20' height='50' align='center'>共找到 <font color=red><%=recordCount%></font> 个品牌信息</td></tr>
  <%for (int i=0;i<BrandList.size();i++) {
  BrandVo vo = new BrandVo();
  
  vo = (BrandVo)BrandList.get(i);
  %>
                  <tr class='tdbg' onMouseOut="this.className='tdbg'" onMouseOver="this.className='tdbgmouseover'">
                    <td width='29' align='center'>
                    <input name='BrandID' type='checkbox' onClick="unselectall()" id='BrandID' value='<%=vo.getId() %>'></td>
                    <td width='509'><a href='Admin_Brand.jsp'><%=vo.getName() %></a></td>
                    <td><a href='Admin_Brand.jsp'><%=vo.getSummary() %></a></td>
                  
                    <td width='168' align='center'>
          
            <a href='Admin_Brand.jsp?Action=Modify&id=<%=vo.getId() %>&type=<%=type%>'>修改</a>&nbsp;
            <a href='Admin_Brand.jsp?Action=del&id=<%=vo.getId() %>' onClick='return confirm("确定要删除此品牌信息吗？");'>删除</a>                    </td>
                  </tr>
<%} %>
              ...
              <table width='100%' border='0' cellpadding='0' cellspacing='0'>
                <tr height='30'>
                  ...
                  <input name='Del' type='submit' value=' 批量删除 ' onClick="document.myform.Action.value='DelAll';return confirm('确定要删除选定的品牌信息吗？');">
                  &nbsp;&nbsp;&nbsp;&nbsp;
                 
                </tr>
              </table>
              
              <a href=Admin_Brand.jsp?Action=Main&pages=<%=pageView.getCurrentPage()-1 %>>上一页</a>  共<%=pageView.getPageSize() %>页  第<%=pageView.getCurrentPage() %>页 <a href=Admin_Brand.jsp?Action=Main&pages=<%=pageView.getCurrentPage()+1 %>>下一页</a>
          </td>
      </form>
      </tr>
    </table>
    ...
    <br>
    <form name='SearchForm' action='Admin_Brand.jsp' method='post'>
    <table width='100%'  border='0' align='center' cellpadding='1' cellspacing='1' class='border'>
      <tr class='tdbg'>
        <td width='113'><b>品牌信息查询：</b></td>
        <td width="1198">
...
<%if("Add".equals(Action)){%>
    <br><table width='100%'><tr><td align='left'>您现在的位置：<a href='Admin_Brand.jsp'>品牌信息管理</a>&nbsp;&gt;&gt;&nbsp;添加品牌信息</td></tr></table>
    <form name='myform' id='myform' action='Admin_Brand.jsp' method='post' onSubmit='return CheckSubmit();'>
    ...
    <table width='100%' height='40'><tr align='center'><td><input type='hidden' name='type' value='<%=type%>'>
        ...
        <input type='submit' name='Submit' value='保存品牌信息信息'>
    </td></tr></table>
    </form>
...
BrandAction getInfo = new BrandAction();
BrandVo vo = new BrandVo();
vo = getInfo.getBrandInfoAction(Integer.parseInt(id));
%>
    <br><table width='100%'><tr><td align='left'>您现在的位置：<a href='Admin_Brand.jsp'>品牌信息管理</a>&nbsp;&gt;&gt;&nbsp;修改品牌信息信息</td></tr></table>
    <form name='myform' id='myform' action='Admin_Brand.jsp' method='post' onSubmit='return CheckSubmit();'>
    <table width='100%'  border='0' align='center' cellpadding='5' cellspacing='1' class='border'>
            <tr class='tdbg'>
                <td height='100' valign='top'>
                    <table width='95%' align='center' cellpadding='2' cellspacing='1' bgcolor='#FFFFFF' id='Tabs' style='display:'>
                        <tr class='tdbg'>
                            <td class='tdbg5' align='right'  width='12%'>品牌信息名称：</td>
...
if("SaveAdd".equals(Action)){
	 ...
	 BrandVo vo = new BrandVo();
	 BrandAction tBrandAction = new BrandAction();
	 ...
	 int status = tBrandAction.insertBrandAction(vo);
	 	 response.sendRedirect("Admin_Brand.jsp?Action=Main");
...
if("SaveModify".equals(Action)){
    String id = request.getParameter("id");
	 ...
	 int status = tBrandAction.updateBrandAction(vo);
	 response.sendRedirect("Admin_Brand.jsp?Action=Main");
}
...
if("del".equals(Action)){
	BrandAction tBrandAction = new BrandAction();
	String id = request.getParameter("id");
 	int status = tBrandAction.delBrandAction(Integer.parseInt(id));
	response.sendRedirect("Admin_Brand.jsp?Action=Main");
}
...
tBrandAction.delBrandAction(BrandID);
response.sendRedirect("Admin_Brand.jsp?Action=Main");
