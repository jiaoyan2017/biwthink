...
<%} %>
              ...
              <a href=Admin_Category.jsp?Action=Main&pages=<%=pageView.getCurrentPage()-1 %>&type=<%=type%>&Keyword=<%=Keyword%>>上一页</a>  共<%=pageView.getPageSize() %>页  第<%=pageView.getCurrentPage() %>页 <a href=Admin_Category.jsp?Action=Main&pages=<%=pageView.getCurrentPage()+1 %>&type=<%=type%>&Keyword=<%=Keyword%>>下一页</a>
          </td>
      </form>
      </tr>
    </table>
