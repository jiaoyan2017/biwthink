...
function movec()//点击移动
{
	...
	$a("leftbut").onmousedown=function(){oli[dnow+1].className=rq[dnow].className="";dnow=(dnow==0)?6:dnow-1;oli[dnow+1].className=rq[dnow].className="show";mv();}
	$a("rightbut").onmousedown=function(){oli[dnow+1].className=rq[dnow].className="";dnow=(dnow==6)?0:dnow+1;oli[dnow+1].className=rq[dnow].className="show";mv();}
...
function HtmlMove(pgtime)
{
	var o=$a("nlnewtop","dd")[0];var mous=false;o.onmouseover=function(){mous=true;};o.onmouseout=function(){mous=false;};
	var om=$a("nlnewtop","div")[0];var li=$a("nlnewtop","ul").length;
	var lih=16;var now=0;var d=setInterval(function(){go_to();},pgtime);om.innerHTML+=om.innerHTML;var ex;
	function go_to()
	{
		clearInterval(ex);if(mous){return;};if(now<li){now++;}else{now=1;o["scrollTop"]=0;};mx=now*lih;
		ex=setInterval(function(){(o["scrollTop"]+1<mx)?(o["scrollTop"]+=1):o["scrollTop"]=mx;if(o["scrollTop"]==mx){clearInterval(ex);}},20);
	}
}
