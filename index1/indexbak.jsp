    ...
    <td height="41" background="images/wmenu_01.jpg"><table width="1280" border="0" cellspacing="0" cellpadding="0">
      <tr>
        ...
        CategoryVo cv = caList.get(i);
         %>
        <td   width="188"><a href="index.jsp?parentid=<%=cv.getId() %>"><div style="background:url(images/hmenu_button_05.gif); height:40px; text-align:center; line-height:40px"><%=cv.getName() %></div></a></td>
        <%} %>
    ...
    <td width="187" align="right" valign="top" background="images/left_02.jpg"><table width="187" border="0" cellspacing="0" cellpadding="0">
      ...
      <%
        ...
        if("".equals(parentid) || parentid==null){
        	...
        	parentid=parentcv.getId()+"";
        }
        
        List<CategoryVo> caChildList = new ArrayList<CategoryVo>();
        if(parentcv!=null){
	       caChildList = ca.getCategoryListByParentIdAction(parentid,"0");
	        for(int i=0;i<caChildList.size();i++){
	        cavo = caChildList.get(i);
       %>
	      <tr>
	        <td><a href="index.jsp?childid=<%=cavo.getId() %>&parentid=<%=parentid%>"><div style="background:url(images/hmenu_button_04.gif); height:40px; text-align:center; line-height:40px"><%=cavo.getName() %></div></a></td>
	      </tr>
	      <tr>
	        <td height="10"></td>
	      </tr>
      ...
      <table width="874" border="0" cellpadding="0" cellspacing="0" background="images/mid_02.jpg">
        <tr>
          ...
          <td width="874" valign="top"><table width="100%" border="0" cellspacing="0" cellpadding="0">
            <tr>
            <%
            if(("".equals(childid) || childid==null)&& caChildList.size()>0){
        		CategoryVo childcv = caChildList.get(0);
        		childid=childcv.getId()+"";
        	}
             %>
              <td> <iframe src="ArticleList.jsp?categoryid=<%=childid %>" name="article" id="article" frameborder="0"  scrolling="no" style="width:874px;height:440px"></iframe></td>
            </tr>
          </table>		 </td>
        </tr>
      </table></td>
